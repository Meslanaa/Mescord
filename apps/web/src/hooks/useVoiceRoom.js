...
const GROUP_CHAT_HISTORY_LIMIT = 240;
const ICE_SERVERS = [
...
export function useVoiceRoom() {
  const [signalingUrl, setSignalingUrl] = useState(() => getRuntimeConnectionConfig().signalingUrl);
  ...
  const socialAuthTokenRef = useRef("");
  const socialUserIdRef = useRef("");
  const activeGroupChannelRef = useRef(null);
  ...
  const [groupChannelContext, setGroupChannelContext] = useState({
    groupId: "",
    channelId: "",
    channelName: "",
  });
  const [groupChannelMessages, setGroupChannelMessages] = useState([]);
  const [groupTypingUsers, setGroupTypingUsers] = useState([]);
  const [groupChannelError, setGroupChannelError] = useState("");
  ...
  const resetState = useCallback(() => {
    ...
    selfIdRef.current = "";
    socialUserIdRef.current = "";
    activeGroupChannelRef.current = null;
    setSocialAuthReady(false);
    ...
    setReactions([]);
    setChatMessages([]);
    setGroupChannelMessages([]);
    setGroupTypingUsers([]);
    setGroupChannelError("");
    setGroupChannelContext({
      groupId: "",
      channelId: "",
      channelName: "",
    });
  ...
  const cleanupConnectionObjects = useCallback(() => {
    ...
    if (socketRef.current) {
      socketRef.current.emit("group_channel_leave");
  ...
  const authenticateSocial = useCallback((token) => {
    const nextToken = typeof token === "string" ? token.trim() : "";
    socialAuthTokenRef.current = nextToken;
    ...
    socketRef.current.emit("social_auth", { token: nextToken });
  ...
  const subscribeGroupChannel = useCallback(
    ({ groupId, channelId }) => {
      ...
      if (!normalizedGroupId || !normalizedChannelId) {
        return { ok: false, reason: "invalid-channel" };
      }

      activeGroupChannelRef.current = {
      ...
      if (!socialAuthTokenRef.current) {
        return { ok: false, reason: "social-auth-required" };
      }

      if (!socialAuthReady) {
        socketRef.current.emit("social_auth", { token: socialAuthTokenRef.current });
      }

      socketRef.current.emit("group_channel_subscribe", {
        groupId: normalizedGroupId,
        channelId: normalizedChannelId,
      });
  ...
  const setGroupTyping = useCallback((isTyping) => {
    if (!socketRef.current || !activeGroupChannelRef.current) {
      return;
    }

    socketRef.current.emit("group_channel_typing", {
      isTyping: Boolean(isTyping),
    });
  }, []);

  const sendGroupChannelMessage = useCallback((text) => {
    let message = typeof text === "string" ? text.trim().slice(0, 1800) : "";
    if (!message) {
      return { ok: false, reason: "empty" };
    }

    if (message.startsWith("/me ")) {
      const body = message.slice(4).trim();
      if (body) {
        const actor = selfProfileRef.current.name || "User";
        message = `*${actor} ${body}*`;
      }
    }
    ...
    socketRef.current.emit("group_channel_message", {
      text: message,
    });
  ...
  const joinRoom = useCallback(
    async ({ roomId, name, color }) => {
      ...
      const finalizeJoin = (ok, message = "") => {
        ...
        if (!ok) {
          resetWithError(message || `Odaya baglanilamadi. Sunucu: ${signalingUrl}`);
        }
      ...
      socket.on("connect", () => {
        ...
        if (socialAuthTokenRef.current) {
          socket.emit("social_auth", { token: socialAuthTokenRef.current });
        }
      });
      ...
      socket.on("disconnect", () => {
        ...
        if (!joinResolved && !activeRoomRef.current) {
          finalizeJoin(false, `Baglanti koptu. Sunucuya ulasilamadi: ${signalingUrl}`);
        }
      });

      socket.on("connect_error", () => {
        finalizeJoin(false, `Sunucuya baglanilamadi: ${signalingUrl}`);
      });

      socket.on("error_event", (payload = {}) => {
        if (!joinResolved) {
          finalizeJoin(false, payload.message || "Beklenmedik bir hata olustu.");
          return;
        }
      ...
      socket.on("social_auth_ok", (payload = {}) => {
        setSocialAuthReady(true);
        socialUserIdRef.current = payload.user?.id || "";

        const activeChannel = activeGroupChannelRef.current;
        if (activeChannel?.groupId && activeChannel?.channelId) {
          socket.emit("group_channel_subscribe", {
      ...
      socket.on("social_auth_error", (payload = {}) => {
        setSocialAuthReady(false);
        setGroupChannelError(payload.message || "Socket social auth basarisiz.");
      });

      socket.on("group_channel_bootstrap", (payload = {}) => {
        setGroupChannelError("");
        setGroupChannelContext({
        ...
        setGroupChannelMessages(Array.isArray(payload.messages) ? payload.messages : []);
      });
      ...
      socket.on("group_channel_message", (payload = {}) => {
        ...
        setGroupChannelMessages((prev) => [...prev, payload].slice(-GROUP_CHAT_HISTORY_LIMIT));
      });

      socket.on("group_channel_typing", (payload = {}) => {
        const users = Array.isArray(payload.users) ? payload.users : [];
        const selfSocialId = socialUserIdRef.current;
        const filtered = selfSocialId ? users.filter((user) => user.id !== selfSocialId) : users;
        setGroupTypingUsers(filtered);
      });

      socket.on("group_channel_error", (payload = {}) => {
        setGroupChannelError(payload.message || "Kanal baglantisinda hata olustu.");
      });

      socket.on("presence_update", () => {
        // Presence updates are consumed by account APIs and can be optionally wired to UI later.
      });

      socket.on(
        "joined_room",
        ({ selfId: joinedSelfId, roomId: joinedRoomId, ownerId: joinedOwnerId, isLocked: joinedRoomLocked, users = [] } = {}) => {
        ...
        setRoomLocked(Boolean(joinedRoomLocked));
      ...
      socket.on("room_meta", ({ ownerId: nextOwnerId, isLocked: nextRoomLocked } = {}) => {
        if (typeof nextOwnerId === "string" && nextOwnerId) {
          setOwnerId(nextOwnerId);
        }

        if (typeof nextRoomLocked === "boolean") {
          setRoomLocked(nextRoomLocked);
        }
      });
      ...
      socket.on("room_lock_changed", ({ isLocked: nextRoomLocked, by } = {}) => {
        if (typeof nextRoomLocked !== "boolean") {
          return;
        }
        ...
        if (by === selfIdRef.current) {
          pushSystemNotice(nextRoomLocked ? "Oda yeni katilimcilara kilitlendi." : "Oda kilidi kaldirildi.");
        }
      });
      ...
      return await new Promise((resolve) => {
        ...
        joinTimeoutId = window.setTimeout(() => {
          finalizeJoin(false, `Odaya baglanti zaman asimina ugradi. Sunucu: ${signalingUrl}`);
        }, 10000);
      });
    },
    [
  ...
  const refreshConnectionConfig = useCallback(() => {
    setSignalingUrl(getRuntimeConnectionConfig().signalingUrl);
  }, []);
  ...
  const setRoomLock = useCallback((locked) => {
    if (!socketRef.current || !activeRoomRef.current) {
      return { ok: false, reason: "not-connected" };
    }

    socketRef.current.emit("set_room_lock", { locked: Boolean(locked) });
    return { ok: true };
  }, []);
